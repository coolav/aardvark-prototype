...
import uib.retrieval.model.*;
import uib.retrieval.util.*;
import uib.retrieval.analyze.*;
...
public class LucenePathIndexRetrievalEngine extends AbstractRetrievalEngine {
    ...
    public List<ResultListEntry> getImagesBySemantics(String xPath, ArrayList objects, String whereToSearch, boolean recursive, JProgressBar progress) {
        ...
        int countGraph = 0;
        try {
            String indexDir = parsePathIndexDirectory(whereToSearch);
    ...
    public List<ResultListEntry> searchForGraph(Graph graph, String whereToSearch) {
        ...
        try {
            QueryParser qParse = new QueryParser(Version.LUCENE_30, "paths", new WhitespaceAnalyzer());        
            Query q = qParse.parse(query);
            TopDocs topDocs = indexSearcher.search(q, 25);
            SAXBuilder builder = new SAXBuilder();
            int maxResults = Math.min(topDocs.totalHits, this.maxResults);
            for (int i = 0; i< maxResults; i++) {
                String[] filenames = indexSearcher.doc(i).getValues("file");
                for (String fileName : filenames) {
                    Element e = builder.build(new FileInputStream(fileName)).getRootElement();
                    ResultListEntry entry = new ResultListEntry((double) topDocs.getMaxScore(), e, fileName);
                    results.add(entry);
                }
            }
        } catch (IOException e) {
    ...
    public List<Node> getNodes(String queryString, String whereToSearch) {
        ...
        try {
            File file = new File((whereToSearch));
            Directory dir = FSDirectory.open(file);
            IndexReader reader = IndexReader.open(dir);
            IndexSearcher searcher = new IndexSearcher(reader);
            String[] fieldsToSearchInForNodes = new String[] {"label", "givenname", "organization", "familyname", "all"};
            MultiFieldQueryParser qParse = new MultiFieldQueryParser(Version.LUCENE_30, 
                    fieldsToSearchInForNodes, new StandardAnalyzer(Version.LUCENE_30));
            ...
            for (int i = 0; i < hitsCount; i++) {
                ...
                sb.append(topDocs.getMaxScore());
                sb.append(": ");
                sb.append(d.get("label"));
                Node node = new Node(Integer.parseInt(d.get("id")), topDocs.getMaxScore(), d.get("label"));
    ...
    public void indexFilesSemantically(String pathToIndex, JLabel statusBar) {
        if (statusBar != null) parent.status.setText("Creating index from semantic annotations");
...
//                            System.out.println(xmlString);
                    ...
                    if (statusBar != null) parent.status.setText("Parsing documents for nodes: " + df.format((float) i / numAllDocsPercent));
            ...
            IndexWriter writer = new IndexWriter(dir, analyzer, createFlag, MaxFieldLength.UNLIMITED);

            if (statusBar != null) parent.status.setText("Creating index for " + element2document.size() + " different available nodes");
...
//                addToDocument(idxDocument, semElement, "//Label/Name", "label", all);
                ...
                Field labelField = new Field("label", elementLabel, Field.Store.YES, Field.Index.ANALYZED);
                ...
                // adding the id:
                idxDocument.add(new Field("id", elementMap.get(xmlString.trim().replaceAll("id=\"id_[0-9]*\"", "")).id + "", Field.Store.YES, Field.Index.NOT_ANALYZED));
            ...
            // ----------------------------------------------------------
            if (statusBar != null) parent.status.setText("Creating and merging of available graphs");
            ...
            */
            ...
            if (statusBar != null) parent.status.setText("Saving index of paths");

            boolean createPathIndexFlag = true;
            String pathIndexDir = parsePathIndexDirectory(pathToIndex);
            
            
            IndexWriter pathIndexWriter = new IndexWriter(dir,  new GraphAnalyzer(), createPathIndexFlag, MaxFieldLength.UNLIMITED);
            ...
            for (Graph graph : graph2document.keySet()) {
                ...
                // adding the graph ...
                idxDocument.add(new Field("graph", graph.toString(), Field.Store.YES, Field.Index.ANALYZED));
//                idxDocument.add(Field.UnIndexed("graph", graph.toString()));
                ...
                idxDocument.add(new Field("paths", sb.toString(), Field.Store.YES, Field.Index.ANALYZED));
    ...
    private static void createIndexDocumentFromSemanticElement(Element semElement, Document idxDocument) {
        ...
        Field field = new Field("all", all.toString(), Field.Store.NO, Field.Index.ANALYZED);
        field.setBoost(0.8f);
    ...
    private static void createIndexDocumentFromSemanticAgent(Element e, Document idxDocument) {
        ...
        idxDocument.add(new Field("givenname", name.getChild("GivenName", mpeg7).getTextTrim(), Field.Store.YES, Field.Index.ANALYZED));
        idxDocument.add(new Field("familyname", name.getChild("FamilyName", mpeg7).getTextTrim(), Field.Store.YES, Field.Index.ANALYZED));
        ...
        if (affiliation != null) {
            ...
            if (organization != null && organization.length() > 0) {
                Field field = new Field("organization", organization, Field.Store.YES, Field.Index.ANALYZED);
                field.setBoost(0.5f);
                idxDocument.add(field);
            }
        }
