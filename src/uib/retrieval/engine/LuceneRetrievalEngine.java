...
//            System.out.println("REGEX: " + regex);
    ...
    public List<ResultListEntry> getImagesByXPathSearch(String xPath, String whereToSearch, boolean recursive, JProgressBar progress) {
        ...
        SAXBuilder builder = new SAXBuilder();
        try {
            File file = new File(parseFulltextIndexDirectory(whereToSearch));
            Directory dir = FSDirectory.open(file);
            IndexReader reader = IndexReader.open(dir);
            StandardAnalyzer analyser = new StandardAnalyzer(Version.LUCENE_20);
            QueryParser qParse = new QueryParser(Version.LUCENE_30, "all", analyser);
            ...
            for (int i = 0; i < hitsCount; i++) {
                ...
                results.add(new ResultListEntry(topDocs.getMaxScore(), e, d.get("file")));
     ...
     * @param statusBar
     */
    public void indexFiles(String pathToIndex) {
        ...
        try {
            IndexWriter writer = new IndexWriter(FSDirectory.open
                    (new File(indexDir)), analyzer, createFlag, MaxFieldLength.UNLIMITED);
...
//                    System.out.println("NumberOfRelations: " + l.size());
                    ...
                    idxDocument.add(new Field("all", all.toString(), Field.Store.NO, Field.Index.ANALYZED));
                    ...
                    if (parent.status != null) {
                        ...
                        status.append('%');
                        parent.status.setText(status.toString());
                    }
    ...
    private void addToDocument(Document document, Element root, String xPath, String fieldName, StringBuilder allContents) {
        ...
        document.add(new Field(fieldName, sw.toString(), Field.Store.YES, Field.Index.ANALYZED));
    }

    public void indexFilesSemantically(String pathToIndex) {
        /*if (parent.status != null) parent.status.setText("Creating index from semantic annotations");
        else if(parent.status == null){
            System.out.println("Creating index from semantic annotations");
        }*/
...
//                            System.out.println(xmlString);
                    ...
                    //if (parent.status != null) parent.status.setText("Parsing documents for nodes: " + df.format((float) i / numAllDocsPercent));
            ...
            //if (parent.status != null) parent.status.setText("Creating index for " + element2document.size() + " different available nodes");
...
//                addToDocument(idxDocument, semElement, "//Label/Name", "label", all);
                ...
                idxDocument.add(new Field("label", elementLabel, Field.Store.YES, Field.Index.ANALYZED));
                ...
                indexWriter.addDocument(idxDocument);
            ...
            // ----------------------------------------------------------
            //if (parent.status != null) parent.status.setText("Creating and merging powersets of available graphs");
...
*/
            ...
            */
            ...
            //if (parent.status != null) parent.status.setText("Storing powersets of available graphs as file");
            String indexFile;
